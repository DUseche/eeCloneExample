 ...
 * <ol>
 * <li>
 *   Create OggInputStream passing in the input stream of the packed ogg file.
 * <li>
 *   Fetch format and sampling rate using getFormat() and getRate(). Use it to
 *   initalize the sound player.
 * <li>
 *   Read the PCM data using one of the read functions, and feed it to 
 *   your player.
 * <p>
 * OggInputStream provides a read(ByteBuffer, int, int) that can be 
 * used to read data directly into a native buffer.
 * <p>
...
public class OggInputStream extends FilterInputStream {
   ...
   * @param input an input stream that reads ogg vorbis data
	 */
   ...
   */
	...
	/**
	 * Decodes a packet.
   * @param packet the packate to decode
   * @return (unknown implementation detail).
   * 
   * pvg: I did not take the time to determine what exactly is returned here,
   *  since it's an implementation detail (by the original author) that
   *  seems to work fine.
	 */
   ...
   * @return 0 or -1 
	...
	private int getNextPacket(Packet packet) throws IOException {
		...
		while (!eos && !fetchedPacket) {
          ...
          // Commented out the assignment statement, since result3 as never
          // used. [pvg]
					//int result3 =  
