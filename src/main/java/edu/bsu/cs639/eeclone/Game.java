...
/**
 * The game logic. A singleton of sorts that is initialized when the constructor
...
public final class Game {
    ...
    /**
     * The sprite manager. Though it is a singleton, we keep a reference for
     * convenience. This manages all of the sprites used in the game
     * <em>except</em>
    ...
    public void setPlayer(Sprite.Player player) {
        this.player = player;
    }
    ...
    /**
     * Set the state of the game. This will cause the old state to be
     * uninstalled and the new state to be installed.
     *
    ...
    /**
     * Get the score object for this game. This wil return null if there is no
     * game happening.
     *
     * @return score
     */
    ...
    private final GameState PLAYING_STATE = new GameState() {
        ...
        public void update() {
            ...
            if (upActive) {
                if (leftActive) {
                    player.move(MovementDirection.UP_LEFT);
                } else if (rightActive) {
                    player.move(MovementDirection.UP_RIGHT);
                } else {
                    player.move(MovementDirection.UP);
                }
            } else if (downActive) {
                if (leftActive) {
                    player.move(MovementDirection.DOWN_LEFT);
                } else if (rightActive) {
                    player.move(MovementDirection.DOWN_RIGHT);
                } else {
                ...
                player.move(MovementDirection.STOP);
            }
    ...
    private final GameState END_OF_GAME_STATE = new GameState() {
        ...
        /**
         * Listens for the press of any key, which signals a change back to the
         * playing state.
         */
        private final KeyListener keyListener = new KeyAdapter() {
            @Override
